...
<x-super-admin-template title="IKU - Super Admin">
    ...
    <x-partials.heading.h2 text="manajemen indikator kinerja utama - indikator kinerja program"
        previousRoute="{{ route('super-admin-iku-ps', ['sk' => $sk['id'], 'ikk' => $ikk['id']]) }}" tooltip>
        @if ($user->isEditor())
            <p>
    ...
    @if ($user->isEditor())
        <x-partials.button.add
            route="{{ route('super-admin-iku-ikp-add', ['sk' => $sk['id'], 'ikk' => $ikk['id'], 'ps' => $ps['id']]) }}"
            style="mr-auto" />
    @endif
                ...
                @foreach ($data as $item)
                    ...
                    <tr class="border-y *:max-w-[500px] *:break-words *:px-5 *:py-2 2xl:*:max-w-[50vw]">
                        ...
                        <td title="{{ $item['name'] }}" class="relative w-max min-w-72 text-left">
                            {{ $item['name'] }}
                            <span
                                title="{{ $item['type'] === 'iku' ? 'Indikator kinerja utama' : 'Indikator kinerja tambahan' }}"
                                class="absolute right-1 top-1 z-10 cursor-default rounded-lg bg-primary/25 p-1 text-xs uppercase text-primary/75">{{ $item['type'] }}</span>
                        </td>
                        ...
                        @if ($user->isEditor())
                            <td title="{{ $item['status'] }}">
                                <label
                                    onclick="pushURL('status-toggle-confirmation', '{{ url(route('super-admin-iku-ikp-status', ['ikp' => $item['id'], 'sk' => $sk['id'], 'ikk' => $ikk['id'], 'ps' => $ps['id']])) }}')"
                            ...
                            <td class="flex items-center justify-center gap-1">
                                <x-partials.button.edit
                                    link="{{ route('super-admin-iku-ikp-edit', ['ikp' => $item['id'], 'sk' => $sk['id'], 'ikk' => $ikk['id'], 'ps' => $ps['id']]) }}" />
    ...
    @if (!count($data))
        <div>
            ...
            <p class="text-center text-red-500 max-lg:text-sm max-md:text-xs">
                {{ $searchQuery !== null ? 'Tidak dapat ditemukan' : 'Tidak ada data indikator kinerja program' }}</p>
        </div>
    @endif
